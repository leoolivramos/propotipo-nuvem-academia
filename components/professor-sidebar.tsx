...
const navItems = [
  ...
  { href: "/professor/professores", label: "Professores", icon: Users },
  { href: "/professor/laboratorios", label: "Laboratórios", icon: FlaskConical },
  { href: "/professor/turmas", label: "Turmas", icon: GraduationCap },
  { href: "/professor/relatorios", label: "Relatórios", icon: FileText },
]
...
export function ProfessorSidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 flex w-16 flex-col items-center bg-sidebar py-4 lg:w-64 lg:items-stretch lg:px-4">
      <Link href="/professor/dashboard" className="flex items-center gap-3 px-2 py-2">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary">
          <Cloud className="h-5 w-5 text-primary-foreground" />
        </div>
        <div className="hidden lg:block">
          <h1 className="text-sm font-bold text-sidebar-foreground">NuvemAcademia</h1>
          <p className="text-[10px] text-sidebar-foreground/50">Professor/Instituição</p>
        </div>
      </Link>
      ...
      <div className="flex flex-col items-center gap-2 lg:items-stretch">
        ...
        <div className="flex items-center gap-3 rounded-lg px-2 py-2">
          ...
          <div className="hidden min-w-0 lg:block">
            ...
            <p className="truncate text-[10px] text-sidebar-foreground/50">Professor/Instituição</p>
          </div>
          <TooltipProvider delayDuration={0}>
            <Tooltip>
              <TooltipTrigger asChild>
